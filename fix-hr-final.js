const fs = require('fs');
const fp = './sops/training/hr-training-data-new.js';
...
c = c.replace(/\{\s*q:\s*"([^"]*)",\s*o:\s*\[([^\]]*)\],\s*c:\s*(\d)\s*\}/g, (match, q, optsStr, ci) => {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (opts[i].length < 45 && opts[i].length < correctLen * 0.5 && !opts[i].includes(' — ')) {
            opts[i] += ' — this reflects the standard operating procedure most organizations follow';
            changed = true;
        }
...
c = c.replace(/\{\s*q:\s*"([^"]*)",\s*o:\s*\[([^\]]*)\],\s*c:\s*(\d)\s*\}/g, (match, q, optsStr, ci) => {
    ...
    for (let i = 0; i < 4; i++) {
        if (i === idx && opts[i].length < shortLen) { shortIdx = i; shortLen = opts[i].length; }
    }
    // Pad the shortest wrong answer
    for (let i = 0; i < 4; i++) {
        if (i === idx) continue;
        if (!opts[i].includes(' — ')) {
...
console.log('Pre-rebalance:', JSON.stringify(dist));
...
console.log(`HR: ${totalQ}q | longest=${cl}(${Math.round(cl / totalQ * 100)}%) | only=${ow} | A:${finalDist[0]} B:${finalDist[1]} C:${finalDist[2]} D:${finalDist[3]}`);
