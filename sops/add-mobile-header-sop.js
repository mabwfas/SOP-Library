...
// CSS to inject (add to the existing <style> block before </style>)
const mobileCss = `
        /* Mobile Header */
        .mobile-header {
            display: none;
...
sopFiles.forEach(file => {
    ...
    // 1. Check if mobile-header already exists
    if (content.includes('mobile-header')) {
        console.log(`Already has mobile-header: ${file}`);
    } else {
        // Add mobile header HTML right after <body>
        content = content.replace('<body>', '<body>' + mobileHeaderHtml);
        changed = true;
        console.log(`Added mobile header HTML to: ${file}`);
    }

    // 2. Add mobile CSS if not present
    if (!content.includes('.mobile-header {')) {
    ...
    // 3. Ensure toggleSidebar function exists
...
console.log('Done adding mobile header to SOP files!');
